...
bool FileHttpRequestHandler::
operator()(const HttpRequest& request, boost::shared_ptr<HttpConnection> connection, const char* begin, const char* end)
{
...
FilesystemHttpRequestHandler::FilesystemHttpRequestHandler(HttpReply::status_type status,
                                                           ...
                                                           bool list_directories,
                                                           const std::vector<HttpHeader>& headers)
    : status_(status), headers_(headers), path_root_(path_root), filesystem_root_(filesystem_root),
      list_directories_(list_directories)
{
}

bool FilesystemHttpRequestHandler::
operator()(const HttpRequest& request, boost::shared_ptr<HttpConnection> connection, const char* begin, const char* end)
{
        ...
        if (boost::filesystem::exists(requested_path))
        {
            if (boost::filesystem::is_directory(requested_path))
            {
                if (list_directories_)
                {
                    ...
                    content << "</body></html>";
                    HttpReply::static_reply(HttpReply::ok, "text/html", content.str(), headers_)(
...
ReplyBuilder::ReplyBuilder(HttpReply::status_type status) : status_(status), headers_(new std::vector<HttpHeader>())
{
}
